...
namespace cowsins
{
    public class WeaponInventorySystem
    {
        ...
        private static readonly int HolsterHash = UnityEngine.Animator.StringToHash("holster");
        ...
        private IEnumerator HolsterCoroutine(WeaponIdentification id, bool playAnim)
        {
            ...
            var animator = id.Animator;

            if (id.weapon != null && playAnim)
            {
            ...
            {
                CowsinsUtilities.PlayAnim(HolsterHash, animator);
        ...
        private IEnumerator HolsterThenSwitchCoroutine(WeaponIdentification current, WeaponIdentification desired, bool playAnim)
        {
            ...
            {
                if (weapon_ != null && weapon_ != current)
                {
            ...
            var currentAnimator = current.Animator;
            if (currentAnimator != null) currentAnimator.enabled = true;

            // Play holster and wait
            yield return context.CoroutineRunner.StartCoroutine(HolsterCoroutine(current, playAnim));
        ...
        public void SelectWeapon()
        {
            ...
            if (current != null)
            {
                ...
                context.CoroutineRunner?.StartCoroutine(HolsterThenSwitchCoroutine(current, desired, shouldPlayHolster));
                return;
            }
        ...
        {
            // Prioritize filling the currently selected slot if it is empty
            if (weaponReference.CurrentWeaponIndex >= 0 && weaponReference.CurrentWeaponIndex < settings.inventorySize)
            {
                if (weaponReference.Inventory[weaponReference.CurrentWeaponIndex] == null)
                {
                    return (false, weaponReference.CurrentWeaponIndex);
                }
            }
