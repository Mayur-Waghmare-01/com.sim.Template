...
public class CrouchSlideBehaviour
{
    ...
    public void Enter()
    {
        ...
        if (CowsinsUtilities.GetLinearVelocity(rb).magnitude >= playerMovement.WalkSpeed && playerMovement.CurrentSpeed > playerMovement.WalkSpeed && playerMovement.Grounded && playerSettings.allowSliding && !context.HasJumped)
        {
    ...
    public void FixedTick()
    {
        ...
        {
            CowsinsUtilities.SetLinearVelocity(rb, Vector3.ProjectOnPlane(CowsinsUtilities.GetLinearVelocity(rb), context.SlopeHit.normal));
        }
        else if (CowsinsUtilities.GetLinearVelocity(rb).y > 0f)
        {
            Vector3 v = CowsinsUtilities.GetLinearVelocity(rb);
            v.y = 0f;
            CowsinsUtilities.SetLinearVelocity(rb, v);
        }
    ...
    {
        return inputManager.Crouching && !playerMovement.IsWallRunning && playerSettings.allowCrouch && (playerMovement.Grounded || !playerMovement.Grounded && playerSettings.allowCrouchWhileJumping) && playerControl.IsControllable && playerControl.IsMovementControllable;
    }
    ...
    private void EndSlide()
    {
        ...
        CowsinsUtilities.SetLinearVelocity(rb, new Vector3(horizontal.magnitude > 0 ? horizontal.normalized.x * Mathf.Max(0, horizontal.magnitude * 0.6f) : 0, vel.y, horizontal.magnitude > 0 ? horizontal.normalized.z * Mathf.Max(0, horizontal.magnitude * 0.6f) : 0));
    }
}
