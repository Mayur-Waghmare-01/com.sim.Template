...
public class GrapplingHookBehaviour
{
    ...
    public void Enter()
    {
        if (!grappleEnabled || isGrappling || !playerControl.IsControllable || !playerControl.IsMovementControllable || weaponReference.MainCamera == null) return;
        RaycastHit hit;
    ...
    public void Tick() 
    {
        ...
        if (playerSettings.grapplingHookMethod == PlayerMovementSettings.GrapplingHookMethod.Linear)
        {
            ...
            Vector3 currentVelocity = CowsinsUtilities.GetLinearVelocity(rb);
            currentVelocity.y = 0;
            CowsinsUtilities.SetLinearVelocity(rb, currentVelocity);
        ...
        else if (playerSettings.grapplingHookMethod == PlayerMovementSettings.GrapplingHookMethod.Combined)
        {
            ...
            {
                float outward = Vector3.Dot(CowsinsUtilities.GetLinearVelocity(rb), -normalizedDirection);
                if (outward > 0f)
                    CowsinsUtilities.SetLinearVelocity(rb, CowsinsUtilities.GetLinearVelocity(rb) + normalizedDirection * outward);
            }
            ...
            if (CowsinsUtilities.GetLinearVelocity(rb).magnitude > maxSpeed)
                CowsinsUtilities.SetLinearVelocity(rb, Vector3.Lerp(CowsinsUtilities.GetLinearVelocity(rb), CowsinsUtilities.GetLinearVelocity(rb).normalized * maxSpeed, Time.deltaTime * 4f));
        }
    }
