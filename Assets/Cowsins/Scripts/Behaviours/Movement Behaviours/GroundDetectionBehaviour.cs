...
namespace cowsins
{
    public class GroundDetectionBehaviour
    {
        ...
        // Slopes
        private float maxSlopeAngle => playerSettings.maxSlopeAngle;
        ...
        private bool UpdateGroundedState(bool foundGroundThisFrame, bool wasGrounded)
        {
            ...
            if (context.HasJumped && CowsinsUtilities.GetLinearVelocity(rb).y > 0.1f && jumpTimer > 0f)
            {
        ...
        {
            return playerMovement.IsCrouching && CowsinsUtilities.GetLinearVelocity(rb).magnitude > playerMovement.WalkSpeed;
        }
