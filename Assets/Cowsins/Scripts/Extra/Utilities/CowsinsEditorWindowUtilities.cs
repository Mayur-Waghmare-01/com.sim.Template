...
namespace cowsins
{
    public static partial class CowsinsEditorWindowUtilities
    {
        ...
        public static void SyncAssetName(Object target, string displayName, string suffix = "")
        {
            if (target == null || string.IsNullOrEmpty(displayName)) return;
            ...
            // Prepare the expected name
            string expectedName = displayName;
            if (!string.IsNullOrEmpty(suffix) && !expectedName.EndsWith(suffix))
                expectedName = $"{expectedName}{suffix}";

            // Sanitize file name (remove invalid characters)
            expectedName = Regex.Replace(expectedName, @"[^a-zA-Z0-9 _-]", "");

            if (currentFileName == expectedName) return;

            Debug.Log($"[Cowsins] Renaming asset at {path} to {expectedName}");
            string result = AssetDatabase.RenameAsset(path, expectedName);
            if (string.IsNullOrEmpty(result))
            {
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh(); // Force Unity to recognize the file system change
            }
        }
    }
}
