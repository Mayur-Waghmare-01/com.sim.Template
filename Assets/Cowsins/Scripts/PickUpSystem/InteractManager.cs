...
namespace cowsins
{
    public class InteractManager : MonoBehaviour, IInteractManagerProvider, IInteractEventsProvider
    {
        ...
        private void Update()
        {
            ...
            if (alreadyInteracted || !playerControl.IsControllable || !playerControl.ActionsControllable)
            {
                if (highlightedInteractable != null) DisableInteractionUI();
                return;
            }
        ...
        private void EnableInteractionUI(Interactable interactable)
        {
            ...
            interactable.Highlight(this.transform);
        ...
        private void PerformInteraction()
        {
            ...
            // If the interactable has a network sync component, delegate to it so
            // the interaction is server-authoritative. Otherwise call Interact() directly.
            if (highlightedInteractable.TryGetComponent<IInteractAuthority>(out var netInteractable))
                netInteractable.RequestInteract(this.transform);
            else
        ...
        private void HandleDrop()
        {
            ...
            if (weaponReferences.Weapon == null || (weaponController.IsReloading && !weaponReferences.Weapon.allowCancelReload) || !weaponController.IsMeleeAvailable || inspecting || !playerControl.IsControllable || !playerControl.ActionsControllable) return;

            WeaponPickeable pick = SpawnService.Spawn(weaponGenericPickeable.gameObject, orientation.Position + orientation.Forward * droppingDistance + transform.right * randomDropOffset, orientation.Rotation).GetComponent<WeaponPickeable>();
        ...
        private void InstantiateAttachmentPickeable(AttachmentIdentifier_SO atcIdentifier)
        {
            ...
            AttachmentPickeable pick = SpawnService.Spawn(attachmentGenericPickeable.gameObject, orientation.Position + orientation.Forward * droppingDistance, orientation.Rotation).GetComponent<AttachmentPickeable>();
