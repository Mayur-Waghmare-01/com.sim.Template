...
namespace cowsins
{
    ...
    public class HitscanShootStyle : IShootStyle
    {
        ...
        public HitscanShootStyle(PlayerDependencies playerDependencies, LayerMask hitLayer)
        {
            ...
            this.playerRoot = playerDependencies.transform.root;
        }
        ...
        private void HitscanShot(float spread)
        {
            ...
            Vector3 dir = CowsinsUtilities.GetSpreadDirection(spread, mainCamera.transform);
            Ray ray = new Ray(mainCamera.transform.position, dir);

            if (Physics.Raycast(ray, out hit, weapon.bulletRange, hitLayer))
            {
                // Ignore hits on the shooter's own colliders
                if (hit.collider.transform.root == playerRoot) return;
                ...
                {
                    if (hitObj.root != newHit.collider.transform.root)
                    {
