...
namespace cowsins
{
    ...
    public class WeaponIdentification : MonoBehaviour
    {
        ...
        public void ReduceOverheatAmmo()
        {
            ...
            if (heatRatio >= 1) isOverheated = true;
        }
        ...
        private int[] cachedShotHashes;

        public int GetCurrentShotAnimation()
        {
            if (cachedShotHashes == null || cachedShotHashes.Length != Mathf.Max(1, weapon.amountOfShootAnimations))
            {
                cachedShotHashes = new int[Mathf.Max(1, weapon.amountOfShootAnimations)];
                cachedShotHashes[0] = Animator.StringToHash("shooting");
                for (int i = 1; i < cachedShotHashes.Length; i++)
                {
                    cachedShotHashes[i] = Animator.StringToHash("shooting" + (i + 1).ToString());
                }
            }

            if (weapon.amountOfShootAnimations <= 1) return cachedShotHashes[0];
            return cachedShotHashes[CurrentShotIndex];
        }
