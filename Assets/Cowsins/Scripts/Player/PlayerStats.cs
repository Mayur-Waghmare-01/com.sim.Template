...
namespace cowsins
{
    ...
    public class PlayerStats : MonoBehaviour, IDamageable, IPlayerStatsProvider, IFallHeightProvider, IPlayerStatsEventsProvider
    {
        ...
        private bool wasHealthOverridden = false;

        private void Start()
        {
            // Apply basic settings 
            if (!wasHealthOverridden)
            {
                health = maxHealth;
                shield = maxShield;
            }
        ...
        {
            float adjustedHealAmount = Mathf.Abs(healAmount * (multipliers != null ? multipliers.HealMultiplier.Value : 1f));
        ...
        /// <summary>
        /// Triggers the death sequence externally (e.g. from network sync).
        /// </summary>
        public void SetDead()
        {
            if (isDead) return;
