...
namespace cowsins
{
    ...
    public class CowsinsButton : Button
    {
        ...
        [SerializeField, Tooltip("Tag used to group selected buttons. Buttons with the same tag will act like radio buttons. Leave empty to ignore.")]
        private string selectionTag;

        [SerializeField, Tooltip("Graphics to enable when this button is selected. Other buttons with the same tag will disable theirs.")]
        private GameObject selectedGraphics;

        [SerializeField, Tooltip("If true, this button will start as selected by default.")]
        private bool isDefault;

        private static List<CowsinsButton> allButtons = new List<CowsinsButton>();
        ...
        protected override void Start()
        {
            ...
            if (selectedGraphics != null) 
                selectedGraphics.SetActive(isDefault);
        }
        ...
        public void ButtonClick()
        {
            ...
            if (buttonType == ButtonType.GameObjectTransition)
            {
                ...
                {
                    Debug.LogWarning("gameObjectToEnable is not assigned for CowsinsButton on " + gameObject.name, this);
                }
            }
        ...
        public void ToggleSelectionGraphics(bool toggle)
        {
            if (selectedGraphics != null) selectedGraphics.SetActive(toggle);
        }
        ...
        public void SelectCategory()
        {
            if (!string.IsNullOrEmpty(selectionTag))
            {
                ...
                {
                    if (btn != null && btn.selectionTag == this.selectionTag && btn.selectedGraphics != null)
                    {
                        btn.selectedGraphics.SetActive(btn == this);
                    }
                }
            }
        }
    ...
    public class CowsinsButtonEditor : ButtonEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("SELECTION SYSTEM", EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(serializedObject.FindProperty("selectionTag"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("selectedGraphics"));
