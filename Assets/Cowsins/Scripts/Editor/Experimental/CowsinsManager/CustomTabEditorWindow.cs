...
namespace cowsins
{
    ...
    public class CustomTabEditorWindow : EditorWindow
    {
        ...
        public static void ShowWindow()
        {
            ...
            // Navigate to Multiplayer FPS Engine tab, Game Modes sub-tab.
            var mpIdx = window.tabs?.FindIndex(t => t.TabName == "Multiplayer FPS Engine") ?? -1;
            if (mpIdx >= 0)
            {
                window.selectedTabIndex = mpIdx;
                var method = window.tabs[mpIdx].GetType().GetMethod("SelectSubTab");
                method?.Invoke(window.tabs[mpIdx], new object[] { "Game Modes" });
            }
            window.Show();
            window.Focus();
            window.Repaint();
        ...
        [MenuItem("Cowsins/Cowsins Manager [Identity Setup]")]
        public static void OpenMultiplayerIdentitySetup()
        {
            var window = GetWindow<CustomTabEditorWindow>();
            if (window.tabs == null || window.tabs.Count == 0)
            {
            ...
            if (mpIdx >= 0)
            {
                window.selectedTabIndex = mpIdx;
                var method = window.tabs[mpIdx].GetType().GetMethod("SelectSubTab");
                method?.Invoke(window.tabs[mpIdx], new object[] { "Player Setup" });
            }
            window.Show();
            window.Focus();
            window.Repaint();
